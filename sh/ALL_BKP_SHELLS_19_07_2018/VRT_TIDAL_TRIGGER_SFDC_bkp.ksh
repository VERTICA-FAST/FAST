					...
					then
						...
						then
							...
							then
								...
								ksh $SHELLDIR/Special_Cases.ksh $line $SUB_AREA
								if [ $? -eq 0 ]
								then
...
mv $TOUCH_FILE_PATH/$SUB_AREA"_SF_OPPORTUNITYLINEITEM_"$YEAR*.txt $TOUCH_FILE_ARCHIVE/
mv $TOUCH_FILE_PATH/$SUB_AREA"_SF_OPPORTUNITYLINEITEM_ID_"$YEAR*.txt $TOUCH_FILE_ARCHIVE/
