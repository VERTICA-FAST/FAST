...
then
	...
	then 
		...
		if [ $EXC_TBL_CNT -eq $TOTAL_TBL_CNT ] 
		then
			...
			echo -e "This is an automatic mail. Please do not respond. This is just to inform you, $SUB_AREA Data Loaded successfully In $ENV for the tables which have data in stage layer.\n\n Total table count in $SUB_AREA = $TOTAL_TBL_CNT \n\n Total table count with data in stage = $LD_TBL_CNT \n\n Actual executed table count = $EXC_TBL_CNT \n\n " | mailx -s "$DAY : $SUB_AREA Data Load successful in $ENV" `cat $CONFIG/mail_recipient.txt`
			if [ $? -eq 0 ]                                                                 			### Checking the result code to check status of mail 
			then
				echo "`$DAYTIME` : $SUB_AREA : Status email has been sent." >> $LOGDIR"/"$SUB_AREA"_Script_Log_"$DAY.log
			else
