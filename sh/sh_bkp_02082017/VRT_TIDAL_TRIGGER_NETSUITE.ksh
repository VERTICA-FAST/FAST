...
then
	...
	for line in `cat $SUB_AREA"_LOAD_ORDER.txt"`; do    ### Reading the filenames in the order for executing respective sqls
		...
		then
			...
			if [ $stg_table_name == "NS_Transaction_lines_USD_Extract" ]
			then 
				export stg_table_count=`/opt/Vertica/bin/vsql -t -h $HOST -p $PORT -d $DB -U $USER -w $PASSWORD -c "select count(*) from swt_rpt_stg.NS_USD_Extract_Closed_Period;"`
				export stg_table_count1=`/opt/Vertica/bin/vsql -t -h $HOST -p $PORT -d $DB -U $USER -w $PASSWORD -c "select count(*) from swt_rpt_stg.NS_USD_Extract_Open_Period;"`
				stg_table_count=$((stg_table_count+stg_table_count1))
			else
