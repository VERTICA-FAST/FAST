...
select id,max(auto_id) as auto_id from swt_rpt_stg.AT_Zuora__Subscription__c where id in (
select id from swt_rpt_stg.AT_Zuora__Subscription__c group by id,LASTMODIFIEDDATE having count(1)>1)
group by id);

delete from swt_rpt_stg.AT_Zuora__Subscription__c where exists(
select 1 from duplicates_records t2 where swt_rpt_stg.AT_Zuora__Subscription__c.id=t2.id and swt_rpt_stg.AT_Zuora__Subscription__c.auto_id<t2.auto_id);
...
SELECT DROP_PARTITIONS('swt_rpt_stg.AT_Zuora__Subscription__c_Hist', TIMESTAMPADD(day,-7,getdate())::date);
