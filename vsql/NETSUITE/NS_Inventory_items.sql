...
select item_id,max(auto_id) as auto_id from swt_rpt_stg.NS_Inventory_items where item_id in (
select item_id from swt_rpt_stg.NS_Inventory_items group by item_id,date_last_modified having count(1)>1)
group by item_id);

delete from swt_rpt_stg.NS_Inventory_items where exists(
select 1 from duplicates_records t2 where swt_rpt_stg.NS_Inventory_items.item_id=t2.item_id and swt_rpt_stg.NS_Inventory_items.auto_id<t2. auto_id);
