...
CREATE LOCAL TEMP TABLE Start_Time_Tmp ON COMMIT PRESERVE ROWS AS select count(*) count, sysdate st from "swt_rpt_stg"."NS_USD_Extract_Closed_Period";
...
select 'NETSUITE','NS_USD_Extract_Closed_Period',sysdate::date,sysdate,null,(select count from Start_Time_Tmp) ,null,'N';
...
INSERT /*+DIRECT*/ INTO "swt_rpt_stg".NS_USD_Extract_Open_Period_Hist SELECT * from "swt_rpt_stg".NS_USD_Extract_Closed_Period;
COMMIT;
...
(SELECT STG.Period_internalid FROM NS_USD_Extract_Closed_Period_stg_Tmp STG);
...
select 'NETSUITE','NS_USD_Extract_Closed_Period',sysdate::date,(select st from Start_Time_Tmp),sysdate,(select count from Start_Time_Tmp) ,(select count(*) from swt_rpt_base.NS_Transaction_lines_USD_Extract where SWT_INS_DT::date = sysdate::date),'Y';
