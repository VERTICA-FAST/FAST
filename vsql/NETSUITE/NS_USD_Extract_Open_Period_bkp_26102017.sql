...
select 'NETSUITE','NS_USD_Extract_Open_Period',sysdate::date,sysdate,null,(select count(*) from "swt_rpt_stg"."NS_USD_Extract_Open_Period") ,null,'N';
...
,SYSDATE AS LD_DT
,SWT_INS_DT 
FROM NS_USD_Extract_Open_Period_base_Tmp WHERE Period_Name in
(SELECT STG.Period_Name FROM NS_USD_Extract_Open_Period_stg_Tmp STG);
