...
select 'PARDOT','PD_Prospect',sysdate::date,sysdate,null,(select count(*) from "swt_rpt_stg"."PD_Prospect") ,null,'N';
...
(
SELECT Prospect_Id, max(Updated_Date) as Updated_Date FROM PD_Prospect_stg_Tmp group by Prospect_Id)
SEGMENTED BY HASH(Prospect_Id,Updated_Date) ALL NODES;
...
(SELECT STG.Prospect_Id FROM PD_Prospect_stg_Tmp_Key STG JOIN PD_Prospect_base_Tmp
ON STG.Prospect_Id = PD_Prospect_base_Tmp.Prospect_Id AND STG.Updated_Date >= PD_Prospect_base_Tmp.Updated_Date);
...
SELECT DISTINCT 
PD_Prospect_stg_Tmp.Prospect_Id
,address_one
,address_two
,annual_revenue
,city
...
,Exclude_from_IP_Warming
,Created_Date
,PD_Prospect_stg_Tmp.Updated_Date
,SYSDATE AS SWT_INS_DT
FROM PD_Prospect_stg_Tmp JOIN PD_Prospect_stg_Tmp_Key ON PD_Prospect_stg_Tmp.Prospect_Id= PD_Prospect_stg_Tmp_Key.Prospect_Id AND PD_Prospect_stg_Tmp.Updated_Date=PD_Prospect_stg_Tmp_Key.Updated_Date
WHERE NOT EXISTS
(SELECT 1 FROM "swt_rpt_base".PD_Prospect BASE
WHERE PD_Prospect_stg_Tmp.Prospect_Id = BASE.Prospect_Id);
...
select 'PARDOT','PD_Prospect',sysdate::date,(select st from Start_Time_Tmp),sysdate,(select count(*) from "swt_rpt_stg"."PD_Prospect") ,(select count(*) from swt_rpt_base.PD_Prospect where SWT_INS_DT::date = sysdate::date),'Y';


Commit;
SELECT PURGE_TABLE('swt_rpt_base.PD_Prospect');
SELECT PURGE_TABLE('swt_rpt_stg.PD_Prospect_Hist');
