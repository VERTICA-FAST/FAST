...
select max(auto_id) as auto_id,ID from swt_rpt_stg.ZU_Payment_Application_Item where ID in(
select ID from swt_rpt_stg.ZU_Payment_Application_Item
group by ID,UpdatedDate having count(1)>1)
group by ID;


delete from swt_rpt_stg.ZU_Payment_Application_Item  where exists(
select 1 from duplicates_records t2 where swt_rpt_stg.ZU_Payment_Application_Item.ID=t2.ID and swt_rpt_stg.ZU_Payment_Application_Item.auto_id<t2.auto_id);
